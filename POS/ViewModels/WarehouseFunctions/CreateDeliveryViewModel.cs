...
namespace POS.ViewModels.WarehouseFunctions
{
    public class CreateDeliveryViewModel : ViewModelBase
    {
        ...
        {
            var ingredientToDelete = _deliveryService.DeliveryCollection.FirstOrDefault(i => i.Ingredient.IngredientId == ingredient.IngredientId);

            if (ingredientToDelete!.Quantity == 1)
            {
